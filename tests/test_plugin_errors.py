...
class PluginErrorTests(unittest.TestCase):
    ...
    def test_factory_not_called_for_incompatible_api(self):
        ...
        entry = ENTRY.replace('"api": 1', '"api": 99').replace("return []", "open({!r}, 'w').close(); return []".format(str(marker)))
    ...
    def test_factory_and_import_failures_are_contextual(self):
        entries = ["raise RuntimeError('import failed')", "raise SystemExit(2)",
        ...
        for index, entry in enumerate(entries):
            location = str(index)
            write_plugin(self.root / location, entry)
            with self.subTest(index=index), self.assertRaisesRegex(PluginError, str(self.root / location)):
                load_plugins(self.root, [location], Registry())
    ...
    def test_duplicate_names_and_providers_leave_core_unchanged(self):
        ...
        self.assertFalse([n for n in set(sys.modules) - before if n.startswith("_etch_plugin_")])
    ...
    def test_doctor_reports_plugin_failure_without_traceback(self):
        ...
        (self.root / "defaults.conf").write_text("{'schema_version': 1, 'plugins': ['missing']}")
        error = io.StringIO()
