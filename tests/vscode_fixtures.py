"""Load the external bundle against an executable fake CLI in a fresh consumer."""

import json
import shutil
import sys
from pathlib import Path
from typing import Any

from etchlib.core import core_registry
from etchlib.plugins.loader import load_plugins
from etchlib.providers.contracts import Context
from etchlib.providers.lifecycle import plan_action
from etchlib.providers.plans import Plan
from tests.planning_fixtures import PlanningFixture

ROOT = Path(__file__).resolve().parents[1]


class VSCodeFixture(PlanningFixture):
    def setUp(self) -> None:
        super().setUp()
        self.bin = self.root / "fake bin"
        self.bin.mkdir()
        self.code = self.bin / "fake code"
        python = str(Path(getattr(sys, "_base_executable", sys.executable)).resolve())
        self.code.write_text(
            "#!" + python + "\n" + (ROOT / "tests/fixtures/vscode_cli.py").read_text()
        )
        self.code.chmod(0o755)
        self.state(extensions=[])
        self.module()
        self.context = Context(self.root, self.root / "modules/demo", "demo", {})
        self.bundle = self.root / "vendor/etch-vscode"
        shutil.copytree(
            ROOT / "plugins/vscode",
            self.bundle,
            ignore=shutil.ignore_patterns("__pycache__"),
        )
        self.write(
            "defaults.conf", {"schema_version": 1, "plugins": ["vendor/etch-vscode"]}
        )
        self.loaded = load_plugins(self.root, ["vendor/etch-vscode"], core_registry())
        self.registry = self.loaded.registry
        self.entry = self.registry.action("vscode")

    def state(self, **values: Any) -> None:
        (self.bin / "state.json").write_text(json.dumps(values))

    def current(self) -> dict[str, Any]:
        value: dict[str, Any] = json.loads((self.bin / "state.json").read_text())
        return value

    def calls(self) -> list[list[str]]:
        path = self.bin / "calls.jsonl"
        return (
            [json.loads(line) for line in path.read_text().splitlines()]
            if path.exists()
            else []
        )

    def vscode_plan(self, extensions: list[str], **options: Any) -> Plan:
        return plan_action(
            self.entry,
            dict(command=str(self.code), extensions=extensions, **options),
            self.context,
        )
