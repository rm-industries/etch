...
class PluginLoadingTests(unittest.TestCase):
    ...
    def test_only_declared_entrypoint_executes(self):
        ...
        (self.root / "declared" / "unrelated.py").write_text("raise RuntimeError('must not run')")
        self.assertEqual(len(load_plugins(self.root, ["declared"], Registry()).plugins), 1)
    ...
    def test_relative_imports_do_not_collide(self):
        ...
        self.assertEqual([e.name for e in loaded.registry.entries()], ["second_fact", "example_fact"])
        self.assertEqual(sys.path, original_path)

    def test_empty_bundle_and_no_declarations(self):
        write_plugin(self.root / "empty", ENTRY.replace("return [ExampleFact()]", "return []"))
        self.assertEqual(load_plugins(self.root, ["empty"], Registry()).registry.entries(), ())
...
'''
        ...
        self.assertEqual(loaded.registry.action("example_action").origin.name, "example")
