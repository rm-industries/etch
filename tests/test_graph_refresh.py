...
class RefreshTests(ConditionFixture, unittest.TestCase):
    def graph_data(self):
        ...
        repository, selections, plans = inputs(module("demo", [{"test": {}, "refresh": ["version"]}, {"test": {}}], facts={"version": {"version": {}}}))
        selections["demo"] = Selection(Result(Outcome.TRUE), (Result(Outcome.TRUE), Result(Outcome.DEFERRED, (ref,))))
    ...
    def test_establish_refresh_condition_chain(self):
        ...
        self.assertEqual(evaluator.evaluate({"fact": {"name": "version", "matches": ">=2"}}).outcome, Outcome.DEFERRED)
    ...
    def test_skipped_or_deferred_producer_cannot_supply_evidence(self):
        ...
        self.assertEqual(build_graph(repository, selections, plans).earlier_refresh(consumer), {})
        plans[producer] = Plan(PlanStatus.CHANGE, "install")
        selections["demo"] = Selection(Result(Outcome.TRUE), (Result(Outcome.DEFERRED), Result(Outcome.DEFERRED, (ref,))))
        self.assertEqual(build_graph(repository, selections, plans).earlier_refresh(consumer), {})

    def test_unordered_producer_needs_explicit_fact_link(self):
        ref = FactRef("consumer", "version")
        repository, selections, plans = inputs(module("consumer", facts={"version": {"version": {}}}), module("installer"))
        consumer, producer = NodeId("consumer", "action", 0), NodeId("installer", "action", 0)
        plans[consumer] = Plan(PlanStatus.CHANGE, "configure", facts=(ref,))
        plans[producer] = Plan(PlanStatus.CHANGE, "install", refresh=(ref,))
        self.assertEqual(build_graph(repository, selections, plans).earlier_refresh(consumer), {})
        graph = build_graph(repository, selections, plans, [FactLink(producer, consumer, ref)])
    ...
    def test_module_gate_can_wait_on_external_producer(self):
        ...
        selections["consumer"] = Selection(Result(Outcome.DEFERRED, (ref,)), (Result(Outcome.DEFERRED, (ref,)),))
        del plans[NodeId("consumer", "action", 0)]
        plans[producer] = Plan(PlanStatus.CHANGE, "install", refresh=(ref,))
        graph = build_graph(repository, selections, plans, [FactLink(producer, gate, ref)])
    ...
    def test_producer_blocked_by_deferred_dependency_is_not_eligible(self):
        ref = FactRef("consumer", "version")
        repository, selections, plans = inputs(module("consumer", facts={"version": {"version": {}}}),
                                               module("installer", requires=["blocked"]), module("blocked"))
        producer, consumer = NodeId("installer", "action", 0), NodeId("consumer", "action", 0)
        selections["blocked"] = Selection(Result(Outcome.DEFERRED), (Result(Outcome.DEFERRED),))
        plans[producer] = Plan(PlanStatus.CHANGE, "install", refresh=(ref,))
        plans[consumer] = Plan(PlanStatus.CHANGE, "configure", facts=(ref,))
        graph = build_graph(repository, selections, plans, [FactLink(producer, consumer, ref)])
    ...
    def test_fact_link_requires_declared_producer_and_consumer(self):
        ...
        with self.assertRaisesRegex(GraphError, "does not consume"):
            build_graph(repository, selections, plans, [FactLink(consumer, producer, ref)])
