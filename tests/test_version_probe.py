...
class VersionProbeTests(unittest.TestCase):
    ...
    def probe(self, code, **options):
        return gather_fact(self.registry.fact("version"), dict(command=[sys.executable, "-S", "-c", code], **options), self.context)

    def test_stdout_and_stderr(self):
        self.assertEqual(self.probe("print('tmux 3.5a')").value, "3.5a")
        self.assertEqual(self.probe("import sys; print('OpenSSH_9.8p1', file=sys.stderr)").value, "9.8p1")

    def test_argv_and_module_cwd_without_shell(self):
        marker = self.root / "should-not-exist"
        code = "import os, sys; assert os.getcwd() == sys.argv[1]; assert sys.argv[2].startswith('$(touch '); print('2.1')"
        config = {"command": [sys.executable, "-S", "-c", code, str(self.root.resolve()), "$(touch {})".format(marker)]}
    ...
    def test_missing_executable_is_unavailable(self):
        result = gather_fact(self.registry.fact("version"), {"command": [str(self.root / "missing"), "-V"]}, self.context)
        self.assertEqual(result.state, FactState.UNAVAILABLE)

    def test_existing_nonexecutable_is_error(self):
        path = self.root / "tool"
        path.write_text("print('1.2')")
        result = gather_fact(self.registry.fact("version"), {"command": [str(path)]}, self.context)
        self.assertEqual(result.state, FactState.ERROR)
        self.assertIn("not executable", result.reason)

    def test_relative_executable_and_relative_path_entries(self):
        (self.root / "python-local").symlink_to(sys.executable)
        with patch.dict(os.environ, {"PATH": "."}):
            for executable in ["./python-local", "python-local"]:
                result = gather_fact(self.registry.fact("version"), {"command": [executable, "-S", "-c", "print('2.1')"]}, self.context)
                self.assertEqual(result.value, "2.1", result.reason)

    def test_timeout_exit_invalid_output_and_output_limit(self):
        for code, options, reason in [("import time; time.sleep(5)", {"timeout": 0.05}, "timed out"),
    ...
    def test_invalid_configuration_prevents_execution(self):
        for config in [{"command": "tmux -V"}, {"command": []}, {"command": [""]},
                       {"command": ["tmux"], "shell": True}, {"command": ["tmux"], "timeout": True},
                       {"command": ["tmux"], "timeout": float("inf")}, {"command": ["tmux"], "timeout": 0}]:
    ...
    def test_version_fact_integrates_with_cache_and_constraints(self):
        store = FactStore(self.registry)
        ref = FactRef("tmux", "tmux_version")
        store.declare(ref, "version", {"command": [sys.executable, "-S", "-c", "print('tmux 3.5a')"]}, self.context)
