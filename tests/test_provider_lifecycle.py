...
class LifecycleTests(unittest.TestCase):
    def setUp(self):
        self.context = Context(Path("/consumer"), Path("/consumer/modules/example"), "example", {})
        self.registry = Registry()

    def test_core_and_external_actions_share_contract(self):
        for origin in [Origin("Etch core", "0.1.0", True), Origin("etch-example", "1.2.0")]:
            with self.subTest(origin=origin):
                ...
                self.assertEqual(plan.claims[0].path, self.context.module_root / "config")
                ...
                self.assertEqual(plan_action(entry, {"value": "desired"}, self.context).status, PlanStatus.SKIP)

    def test_validation_failure_prevents_inspection(self):
        action = MemoryAction()
        self.registry.register(Origin("example", "1"), [action])
        with self.assertRaisesRegex(ProviderError, "validate failed: value is required"):
    ...
    def test_invalid_result_types(self):
        for method, value in [("validate", False), ("inspect", {}), ("plan", {})]:
            action = MemoryAction()
            setattr(action, method, lambda *args: value)
            registry = Registry()
            registry.register(Origin("example", "1"), [action])
            with self.subTest(method=method), self.assertRaisesRegex(ProviderError, method):
    ...
    def test_fact_contract_and_wrong_kind(self):
        ...
        self.assertEqual(gather_fact(entry, {"value": 42}, self.context).state, FactState.VALUE)
