...
class BootstrapTests(unittest.TestCase):
    ...
    def vendor(self, destination=None):
        ...
        shutil.copytree(ROOT / "etchlib", destination / "etchlib", ignore=shutil.ignore_patterns("__pycache__"))
        return destination

    def run_install(self, *args, consumer=None):
        return subprocess.run([str((consumer or self.consumer) / "install"), *args],
...
"""
        result = subprocess.run([sys.executable, "-S", "-c", probe, str(vendor), str(self.consumer)],
                                cwd=self.root, env=self.env, text=True, capture_output=True)
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertIn("Configuration structure OK", result.stdout)

    @unittest.skipUnless(shutil.which("git"), "Git is needed to construct the submodule distribution fixture")
    def test_recursive_clone_with_pinned_submodule(self):
        git = shutil.which("git")
        env = dict(os.environ, GIT_CONFIG_GLOBAL=os.devnull, GIT_CONFIG_NOSYSTEM="1",
                   GIT_AUTHOR_NAME="Etch test", GIT_AUTHOR_EMAIL="test@example.invalid",
                   GIT_COMMITTER_NAME="Etch test", GIT_COMMITTER_EMAIL="test@example.invalid")

        def run_git(cwd, *args):
            return subprocess.run([git, "-c", "protocol.file.allow=always", *args],
                                  cwd=cwd, env=env, text=True, capture_output=True, check=True).stdout.strip()
