...
def main(argv: Optional[List[str]] = None) -> int:
    parser = argparse.ArgumentParser(prog="etch", description="Make your environment yours.")
    parser.add_argument("--version", action="version", version="Etch " + __version__)
    commands = parser.add_subparsers(dest="command")
    doctor = commands.add_parser("doctor", help="check repository configuration structure")
    doctor.add_argument("modules", nargs="*", help="module names (default: all discovered modules)")
    doctor.add_argument("--profile", help="select an ordered profile")
    doctor.add_argument("--repo", type=Path, default=Path.cwd(), help="consumer repository root (default: current directory)")
