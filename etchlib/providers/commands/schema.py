...
def normalize(name, config, context):
    ...
    for value in values:
        ...
        if "description" in options and (not isinstance(options["description"], str) or not options["description"].strip()):
        ...
        if check is not None:
            ...
            if kind not in ("command", "path_exists", "file_exists", "directory_exists") or not isinstance(query, str) or not query or "\x00" in query:
...
def _argv(value, allow_empty):
    if not isinstance(value, list) or (not value and not allow_empty) or any(not isinstance(v, str) or "\x00" in v for v in value):
