...
class CommandProvider:
    ...
    def inspect(self, config: Any, context: Context) -> Inspection:
        ...
        return Inspection(
            InspectionState.UNKNOWN if ready else InspectionState.SATISFIED,
            tuple(ready),
        )
    ...
    def plan(self, config: Any, observation: Inspection, context: Context) -> Plan:
        ...
        resources = (("sudo-interactive",) if elevated else ()) + (
            ("stdin-interactive",) if interactive else ()
        )
        return Plan(
        ...
        for item in plan.payload:
            if not checked(item, context):
                ...
                if item.get("sudo", False) and not context.elevation_allowed:
                    raise ValueError(
                        "command privilege escalation has not been authorized"
                    )
        changed = False
        for item in plan.payload:
            if not checked(item, context):
                ...
                changed = True
        return ApplyResult(
            changed, "Commands ran" if changed else "Checks already satisfied"
        )
