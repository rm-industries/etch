...
class Registry:
    ...
    def register(
        self, origin: Origin, actions: Iterable[Any] = (), facts: Iterable[Any] = ()
    ) -> None:
        ...
        for kind, providers, methods in (
            ...
            for provider in providers:
                ...
                if not isinstance(name, str) or not re.fullmatch(
                    ...
                    raise ProviderError(
                        "{}: invalid {} provider name {!r}".format(
                            origin.name, kind, name
                        )
                    )
                if kind == "action" and name in (
                    "when",
                    "requires",
                    "after",
                    "refresh",
                    ...
                    raise ProviderError(
                        "action provider name {!r} is reserved for metadata".format(
                            name
                        )
                    )
                ...
                if previous is not None:
                    raise ProviderError(
                        "provider {!r} from {} conflicts with {} {} ({})".format(
                            name,
                            origin.name,
                            previous.kind,
                ...
                if missing:
                    raise ProviderError(
                        "{}: {} provider {!r} lacks callable {}".format(
    ...
    def lookup(self, name: str, kind: str) -> Registration:
        ...
        if entry.kind != kind:
            raise ProviderError(
                "provider {!r} is a {}, not an {}".format(name, entry.kind, kind)
            )
        return entry
