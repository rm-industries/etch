...
def options(config, context):
    ...
    if not isinstance(paths, list) or not paths or not isinstance(obsolete, list):
        raise ValueError(
            "clean requires a nonempty paths list and optional obsolete list"
        )
    ...
    if any(path.parent not in roots for path in retired):
        raise ValueError(
            "obsolete destinations must be immediate children of selected directories"
        )
...
class CleanProvider:
    ...
    def inspect(self, config, context):
        ...
        for root in roots:
            ...
            if state != "directory":
                raise ValueError(
                    "clean roots must be real directories, not files or symlinks: {}".format(
                        root
                    )
                )
        ...
        return Inspection(
            InspectionState.CHANGE if removals else InspectionState.SATISFIED,
            tuple(removals),
        )
    ...
    def plan(self, config, observation, context):
        return Plan(
            ...
            + ", ".join(str(item.path) for item in observation.data)
            if observation.data
            else "No proven obsolete or broken managed links",
    ...
    def apply(self, plan, context):
        ...
        for item in plan.payload:
            ...
            if kind(item.path) == "missing":
                continue
            if (
                not owned(context.repo_root, item.path)
                or identity(item.path) != item.proof
            ):
                raise ValueError(
                    "managed link changed since planning: {}".format(item.path)
                )
        ...
        for item in pending:
            ...
            changed = True
        return ApplyResult(
            changed, "Managed links removed" if changed else "No managed links removed"
        )
