...
def entries(config, context):
    ...
    for target, value in config.items():
        ...
        options = compose_defaults("link", context.defaults.get("link", {}), options, ("create", "relink"))
        ...
        result.append(Link(destination(target, context.repo_root), source,
                           options.get("create", False), options.get("relink", False)))
    ...
    for link in result:
        for other in result:
            ...
            if link.destination == other.source or link.destination in other.source.parents:
                raise ValueError("link destination would replace a source in the same action")
    return tuple(result)
...
def needed(link):
    ...
    if link.destination == link.source or link.destination in link.source.parents:
        raise ValueError("link destination would replace its own source: {}".format(link.destination))
    ...
    if state == "missing":
        return True
    if state != "link":
        raise ValueError("refusing to replace file or directory: {}".format(link.destination))
    if link.destination.resolve() == link.source:
        return False
    if not link.relink:
        raise ValueError("different or broken link: {} (enable relink)".format(link.destination))
    return True
...
class LinkProvider:
    ...
    def inspect(self, config, context):
        ...
        return Inspection(InspectionState.CHANGE if any(changes) else InspectionState.SATISFIED, links)
    ...
    def plan(self, config, observation, context):
        ...
        for link in observation.data:
            ...
            if link.create:
                claims.append(PathClaim(link.destination.parent, ClaimKind.SHARED_DIRECTORY))
        return Plan(PlanStatus.SKIP if observation.state is InspectionState.SATISFIED else PlanStatus.CHANGE,
                    ...
                    payload=observation.data, claims=tuple(claims))

    def apply(self, plan, context):
        for link in plan.payload:
            if destination(str(link.destination), context.repo_root) != link.destination:
