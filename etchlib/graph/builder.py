...
def build_graph(repository, selections, plans=None, fact_links=()):
    ...
    for module in repository.modules:
        ...
        if len(selected.actions) != len(actions):
            raise GraphError(
                "{}: condition selection has the wrong action count".format(module.name)
            )
        if selected.module.outcome is not Outcome.TRUE and any(
            result.outcome is not selected.module.outcome for result in selected.actions
        ):
            raise GraphError(
                "{}: action outcomes must preserve an inactive or deferred module gate".format(
                    module.name
                )
            )
        ...
        if selected.module.outcome is Outcome.TRUE:
            dependencies.append(
                (
                    start,
                    module.config.get("requires", []),
                    module.config.get("after", []),
                )
            )
        for index, (action, selection) in enumerate(zip(actions, selected.actions)):
            ...
            tails = [key]
            if (
                selected.module.outcome is Outcome.TRUE
                and selection.outcome is Outcome.TRUE
            ):
                dependencies.append(
                    (
                        key,
                        tuple(action.get("requires", []))
                        + (plan.requires if plan else ()),
                        tuple(action.get("after", [])) + (plan.after if plan else ()),
                    )
                )
            inputs = set(selection.waiting) | set(plan.facts if plan else ())
            refresh = set(
                FactRef(module.name, name) for name in action.get("refresh", [])
            )
            refresh.update(plan.refresh if plan else ())
            if (inputs | refresh) - declared:
                raise GraphError(
                    "{}: undeclared fact references: {}".format(
                        key, (inputs | refresh) - declared
                    )
                )
    ...
    for link in fact_links:
        if link.producer.kind != "action" or link.consumer.kind not in (
            "action",
            "start",
        ):
            raise GraphError(
                "fact links require an action producer and action/module-gate consumer"
            )
