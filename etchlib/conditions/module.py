...
def select_module(module, evaluator) -> Selection:
    if evaluator.context.module_name != module.name or evaluator.context.module_root != module.root:
        raise ConditionError("module selection requires an evaluator scoped to that module")
    ...
    gate = evaluator.evaluate(module.config["when"]) if "when" in module.config else Result(Outcome.TRUE)
    if gate.outcome is not Outcome.TRUE:
        return Selection(gate, tuple(gate for _ in actions))
    results = tuple(evaluator.evaluate(action["when"]) if "when" in action else Result(Outcome.TRUE)
