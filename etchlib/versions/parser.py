...
class Version:
    ...
    def __post_init__(self):
        if not isinstance(self.numbers, tuple) or not self.numbers or any(type(n) is not int or n < 0 for n in self.numbers):
            raise ValueError("version components must be nonnegative integers")
        if not isinstance(self.suffix, str) or re.fullmatch(r"[a-z]?", self.suffix) is None:
            raise ValueError("version suffix must be a single lowercase letter")
        if type(self.suffix_number) is not int or self.suffix_number < 0 or (not self.suffix and self.suffix_number):
    ...
    def __lt__(self, other):
        ...
        return (self.numbers, self.suffix, self.suffix_number) < (other.numbers, other.suffix, other.suffix_number)
...
def extract_version(output: str) -> str:
    ...
    match = re.search(r"(?<![A-Za-z0-9.+-])v?[0-9]+(?:\.[0-9]+)+(?:[A-Za-z0-9.+-]*)", line)
    if match is None:
