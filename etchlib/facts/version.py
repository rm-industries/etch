...
class VersionProbe:
    ...
    def validate(self, config, context):
        ...
        if type(timeout) not in (int, float) or not math.isfinite(timeout) or timeout <= 0:
            raise ValueError("version timeout must be a positive finite number of seconds")
    ...
    def gather(self, config, context):
        ...
        try:
            ...
            if "/" in executable:
                ...
                executable = str(path if path.is_absolute() else context.module_root / path)
            # Resolve relative PATH entries against the same cwd used by the child.
            search = os.pathsep.join(str(context.module_root / p) if not os.path.isabs(p) else p
                                     for p in os.get_exec_path())
            found = shutil.which(executable, path=search)
            if found is None:
                candidates = [Path(executable)] if "/" in executable else [Path(p) / executable for p in search.split(os.pathsep)]
                if any(p.exists() for p in candidates):
                    return FactResult(FactState.ERROR, reason="command {!r} exists but is not executable".format(argv[0]))
                return FactResult(FactState.UNAVAILABLE, reason="command {!r} not found".format(argv[0]))
            argv[0] = os.path.abspath(found)
            # Spool output to files: noisy commands cannot fill memory while running.
            with tempfile.TemporaryFile() as stdout, tempfile.TemporaryFile() as stderr:
                completed = subprocess.run(argv, cwd=context.module_root, stdin=subprocess.DEVNULL,
                                           stdout=stdout, stderr=stderr, timeout=config.get("timeout", 10), shell=False)
                if completed.returncode:
                    return FactResult(FactState.ERROR, reason="version command exited with status {}".format(completed.returncode))
                stdout.seek(0)
                stderr.seek(0)
                out, err = stdout.read(OUTPUT_LIMIT + 1), stderr.read(OUTPUT_LIMIT + 1)
            if len(out) > OUTPUT_LIMIT or len(err) > OUTPUT_LIMIT:
                return FactResult(FactState.ERROR, reason="version output exceeds 64 KiB per stream")
            ...
            return FactResult(FactState.ERROR, reason="version probe failed: {}".format(exc))
